class AccountToSemestersController < ApplicationController
  ...
  def show
    @account_to_semester = AccountToSemester.includes(:operations).find(params[:id])
  end

  def index
    if params[:years].nil? or params[:years].empty?
      @account_to_semesters = AccountToSemester.includes(:faculty).where(year: year_today)
    else
      @account_to_semesters = AccountToSemester.includes(:faculty).where(year: params[:years].to_s)
    end
    unless current_user.faculty.name == "all"
      @account_to_semesters = @account_to_semesters.find_all{|x| x.faculty_id == current_user.faculty_id}
    end
  end
